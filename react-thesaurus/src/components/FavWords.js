...
import { thesaurusApiUrl } from "../config";
...
const FavWords = (props) => {
  ...
  const getFavouriteWords = () => {
    ...
    fetch(thesaurusApiUrl, requestOptions)
      .then(res => res.json())
      .then(data => {
       // console.log(data);
       dispatch(wordActions.updateFavWords(data.favouriteWords));
      })
  ...
  if(duplicate) return;
   ...
   var reqBody = {
    favouriteWord: `${enteredValue}`,
    httpMethod: "POST"
  } 
    
//onsole.log("The req body stringified");
//console.log(JSON.stringify(...reqBody));

  
   let wordToAdd = "";
   wordToAdd = enteredValue;
    const requestOptions = {
      method: 'POST',
